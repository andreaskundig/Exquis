...
function(noise, paper, idu, shapes, ShapeGrid,wp){
    ...
    const createShape = (paper, topLeft, width) => {
      ...
      const shapeSize = new paper.Size(side * 0.65,side*1.2).multiply(1.2);
    ...
    return {
        ...
        draw: function (context, borders){
            ...
            g.forEach((square,x,y,index) =>{
             ...
             const fakeBorderScaling = {};
             ["north", "east", "south", "west"].forEach(d => {
                 fakeBorderScaling[d] = [];
             })
             for(let i=0; i < elementsPerSide; i++){
                 fakeBorderScaling.north.push(computeScale(i,-1));
                 fakeBorderScaling.east.push(computeScale(elementsPerSide,i));
                 fakeBorderScaling.south.push(computeScale(i,elementsPerSide));
                 fakeBorderScaling.west.push(computeScale(-1,i));
             }
             g.forEach((square,x,y,index) => {
